...
const refOf = (stop: Place & { status?: string }): StopRef => ({
  id: stop.id,
...
export function decodeJourney(raw: unknown): Journey | null {
  ...
  try {
    ...
    if (!x || x.v !== 1 || !Array.isArray(x.pending) || !Array.isArray(x.done) || !Array.isArray(x.skipped) ||
      x.pending.length > placeCount || x.done.length > placeCount || x.skipped.length > placeCount ||
      !validInt(x.clock, 1439) || !validInt(x.minutes, 480) || !validInt(x.cash, 10000) ||
      !["", "holiday", "golden"].includes(x.current)) return null;

    const validRef = (ref: StopRef) => ref && typeof ref.id === "string" && placeCatalog.has(ref.id) &&
      (ref.q === undefined || (ref.q === 1 && placeCatalog.get(ref.id)?.category === "food"));

    if (!x.pending.every(validRef) ||
      !x.done.every((ref: Visit) => validRef(ref) && validInt(ref.t, 1439) && validInt(ref.d, 480) && validInt(ref.p, 10000) && validInt(ref.w, 120)) ||
      !x.skipped.every((id: unknown) => typeof id === "string" && placeCatalog.has(id))) return null;

    const all = [
      ...x.pending.map((ref: StopRef) => ref.id),
      ...x.done.map((ref: StopRef) => ref.id),
      ...x.skipped
    ];
    ...
    return {
      v: 1,
      pending: x.pending.map(cleanRef),
      done: x.done.map((ref: Visit) => ({ ...cleanRef(ref), t: ref.t, d: ref.d, p: ref.p, w: ref.w })),
      skipped: x.skipped,
      clock: x.clock,
      minutes: x.minutes,
      cash: x.cash,
