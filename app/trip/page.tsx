...
export default async function TripPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><Header/><main id="main-content" className="page trip-page">
    ...
    {closedNotices.map(notice => <p role="status" className="applied-banner" key={notice.id}><strong>{notice.name} 已闭店。</strong> {notice.closureLabel}。{notice.alternatives.length ? `可考虑：${notice.alternatives.map(item => item.name).join("、")}。` : "当前地点库暂没有合适替代项。"} <a href={notice.sourceUrl} target="_blank" rel="noreferrer">{notice.sourceLabel}<Icon name="external" size={13}/></a></p>)}
