...
export default function GuidePage() {
  return <><Header/><main id="main-content" className="page guide-page"><Link className="back-link" href="/"><Icon name="back" size={17}/>回去规划今天</Link>
    ...
    <div className="guide-facts"><article><span>01 / 已有连接</span><h2>双商圈已连通</h2><p>中山+报道，多个连通口于2026年8月18日启用。路线按商场分组，跨区时单独留出换区时间。</p><a href={sources.connection} target="_blank" rel="noreferrer">中山+ · 2026-08-18 <Icon name="external" size={14}/></a></article><article><span>02 / 文化停留</span><h2>书店值得慢一点</h2><p>南方日报介绍了博雅书店的中山地方文献及港版图书展区。浏览与购书分开估算，避免把每一次停留都变成消费。</p><a href={sources.books} target="_blank" rel="noreferrer">南方日报 · 2026-04-24 <Icon name="external" size={14}/></a></article><article><span>03 / 家庭出行</span><h2>为途中休息留空间</h2><p>2025年报道提及金鹰广场的母婴室、亲子卫生间等设施。具体楼层与可用情况尚未核实，可向商场服务台询问。</p><a href={sources.services} target="_blank" rel="noreferrer">南方日报 · 2025-09-12 <Icon name="external" size={14}/></a></article></div>
    <section className="guide-section"><div className="section-heading"><h2>出发前，先知道这些</h2><span className="section-note">公开事实 × 规划估算</span></div><div className="arrival-grid"><article><Icon name="pin"/><h3>找对目的地</h3><p>假日广场：兴中道6号。石岐万象汇：孙文东路28号。地点卡可打开高德搜索；导航到商场后，再按现场导视寻找铺位。</p></article><article><Icon name="rain"/><h3>雨天与少走路</h3><p>连通口开放不等于已核实全程遮雨或无台阶。雨天优先单一商场；途中选“少走路”会优先留在你标记的商场。</p></article><article><Icon name="wallet"/><h3>预算留给实际需要</h3><p>餐饮、游玩费用是可调整的人均规划预留，不是门票报价。浏览书店与逛店不预设购物消费；购买后可手动更新剩余预算。</p></article><article><Icon name="clock"/><h3>把未知留给现场确认</h3><p>未接入实时营业、排队、车位、票价或室内导航。停车优惠与会员权益请查看当期规则；不会把历史活动写成正在进行。</p></article></div></section>
    <section className="guide-section"><div className="section-heading"><h2>路线里的地点，从哪里来？</h2></div><p className="summary-copy">下面列出资料支持的地址或品牌信息。历史公告和平台收录均不代表今天营业；缺少可靠铺位时直接标明待确认。</p><div className="place-directory">{places.filter(place => !["start", "connector", "rest", "shopping"].includes(place.category) || ["nitori", "hema", "popmart"].includes(place.id)).map(place => <article key={place.id}><div><small>{place.mall} · {place.floor}</small><h3>{place.name}</h3><p>{place.evidenceNote || "公开资料可支持该地点的地址或品牌信息；营业、活动和消费以现场为准。"}</p></div><a href={place.sourceUrl} target="_blank" rel="noreferrer">{place.sourceLabel}<Icon name="external" size={14}/></a></article>)}</div></section>
    <section className="guide-section"><div className="section-heading"><h2>试着处理一次途中变化</h2><span className="section-note">可复现演示 · 非试用记录</span></div><div className="demo-grid">{cases.map((item, index) => {
