...
export default function PlannerForm({ initial }: { initial: PlanInput }) {
  ...
  return <form action={generateTrip} className="planner-panel v4-planner" id="planner">
    ...
    <div className="panel-heading">
      ...
      <div>
        <p className="panel-kicker">TODAY'S PERFECT DAY</p>
        <h2>今天，想怎么逛？</h2>
        <p>说出同行人、时间和偏好，先给你一条刚好的路线。</p>
      </div>
    </div>

    <div className="request-field">
      <label htmlFor="request"><span className="field-number">01</span><strong>先简单说说你的想法</strong><em>选填</em></label>
      <textarea
        id="request"
        name="request"
        value={request}
        onChange={event => { setRequest(event.target.value); setExample(""); }}
        maxLength={600}
        rows={3}
        placeholder="比如：陪爸妈逛两小时，想逛宜得利，不喝咖啡，尽量室内"
        aria-describedby="request-help"
      />
      <div className="request-help"><p id="request-help">也可以直接选下方偏好，文字描述优先。</p><small>{request.length}/600</small></div>
    </div>

    <div className="example-chips" aria-label="试填一种出行想法">
      {EXAMPLES.map(item => <button
        key={item.label}
        type="button"
        aria-pressed={example === item.label}
        onClick={() => {
          setRequest(item.request);
          setScene(item.scene);
          setDuration(item.duration);
          setBudget(item.budget);
          setWalking(item.walking);
          setExample(item.label);
        }}
      ><Icon name="sparkles" size={13}/>{item.label}</button>)}
    </div>

    <fieldset>
      <legend><span className="step-number">02</span>和谁一起逛？</legend>
      <div className="scene-grid">
        {SCENES.map(([value, icon, label]) => <label className="scene-option" key={value}>
          <input type="radio" name="scene-choice" value={value} checked={visibleScene === value} onChange={() => { setScene(value); setExample(""); }}/>
    ...
    <fieldset>
      <legend><span className="step-number">03</span>时间、预算和步行偏好？</legend>
    ...
    <SubmitButton/>
    <div className="form-reassurance"><Icon name="check" size={14}/>途中变了，也只调整还没发生的部分</div>
  </form>;
}
