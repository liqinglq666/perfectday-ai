...
通过校验的 AI 字段保持其语义结果，本地规则不再重新提取数字、推断天气或用餐厅名称白名单删除偏好。模型必须输出 `budgetLimit`（明确预算整数，未给金额为 `null`）和 `mealIntent`（`none` / `generic` / `specific`）。泛称吃饭保留一个正餐位置；明确食物偏好允许同义表达，例如“面条”可选择味千拉面。缺少或错误的字段会触发本地回退。

本地回退按短句和用途区分数字：“逛90分钟，坐车两小时”只采用90分钟；“预算不限/没想好，逛两小时”保留表单预算。模板经预算、时间、商场和室内约束筛空后，初始规划会再查找符合条件的地点；没有可行地点时仍显示空路线。途中重排保持已完成记录，不自动补回已跳过的地点。
